...
class Trie:
	...
	def getAllConstrainedWords(self,minLength,maxLength,letters,wordList,word = ''):
		...
		for i in range(len(letters)):
			if letters[i] not in self.nextLetters:
				continue
			self.nextLetters[letters[i]].getAllConstrainedWords(minLength,maxLength,letters[:i] + letters[i+1:],wordList,currentWord)
